...
async def send_bit(dut, bit_value: int, op: int):
    # Drive inputs on falling edge so they are stable before the next rising edge.
    await FallingEdge(dut.clk)
    dut.ui_in.value = pack_ui(bit_value, op)
    await RisingEdge(dut.clk)

    # Important for gate-level simulation:
    # the synthesized netlist is compiled with UNIT_DELAY=#1, so outputs/internal
    # signals settle after the clock edge, not exactly at the clock edge.
...
async def run_alu_test(dut, a: int, b: int, op: int, expected: int, name: str):
    ...
    # Done is expected after the 14th rising edge. In gate-level simulation,
    # wait a little more so the delayed gates settle.
...
async def test_project(dut):
    ...
    # Do not wait additional full clock cycles after releasing reset.
    # The ALU has no valid/enable input; every rising edge with rst_n=1 is data.
